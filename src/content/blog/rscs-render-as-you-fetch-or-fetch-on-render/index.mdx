...
1. **Render-as-you-fetch**: In this approach, fetching starts _before_ rendering does. Think of it as “fetch triggers render.” The data fetch usually happens higher up in the component tree so multiple requests can run at the same time. Components can start showing up right away, maybe with loading states until the data arrives.

2. **Fetch-on-render**: This one is more like “render triggers fetch.” Each component takes care of its own data fetching, usually inside its own logic. That makes the code modular and self-contained, but it can cause a network waterfall on the client. This waterfall is even worse when you’ve got several nested components that depend on each other’s data.
