...
This is not hidden; it's mentioned in the docs and also covered in Lee's video. Additionally, this is not a Next.js cache, it's a React cache designed for deduplication and does not persist. For persistent caching in Next.js, you can utilize the `unstable_cache` function.
...
We wouldn't want a new cache to get created on each rerender, but more importantly, we want that cache accessible outside of that component.
...
Yes, the default dev server has performance issues. Especially compared to esbuild/Vite. But, turbopack works well now and it's much a much better experience. To enable turbopack, include this in the dev script: `"dev": "next dev --turbo",`
...
As I mentioned before, server-side rendering is about more than just SEO. Also, with client components we can still put as much code on the client as we like, but now we get the additional benefit of SSR and RSC's.
